...
for (let d of document.getElementsByClassName('counter')) {
  let i = 1;
  window.setInterval(function() {
    ...
    d.innerHTML = `starting database dump!\n${minute_str}:${seconds_str}█`
    i += 1
    if (i > 90) {
      i = 0
    }
...
for (let d of document.getElementsByClassName('counter2')) {
  let i = 1;
  window.setInterval(function() {
    let minutes = Math.floor(i / 60)
    let seconds = i % 60
    let minute_str = minutes < 10 ? '0' + minutes : minutes
    let seconds_str = seconds < 10 ? '0' + seconds : seconds
    d.innerHTML = `pg_dump bigdb -f bigdb.dump ... ${minute_str}:${seconds_str}█`
    i += 1
    if (i > 10) {
      d.innerHTML = `pg_dump bigdb -f bigdb.dump ... done`
      i = 0
    }
  }, 1000)
}
